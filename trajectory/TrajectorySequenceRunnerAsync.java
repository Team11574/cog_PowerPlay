...
import incognito.cog.trajectory.sequencesegment.SequenceSegment;
import incognito.cog.trajectory.sequencesegment.TrajectorySegment;
import incognito.cog.trajectory.sequencesegment.TurnSegment;
import incognito.cog.trajectory.sequencesegment.WaitSegment;
import incognito.cog.util.roadrunner.DashboardUtil;
import incognito.cog.util.roadrunner.LogFiles;
import incognito.teamcode.config.DriveConstants;

@Config
public class TrajectorySequenceRunnerAsync {
    ...
    public void on(Event event, Runnable callback) {
        Objects.requireNonNull(callbacks.get(event)).add(callback);
    }

    private void dispatchCallback(Event event) {
        for (Runnable callback : Objects.requireNonNull(callbacks.get(event))) {
            callback.run();
        }
    }
    ...
    DriveSignal update(Pose2d poseEstimate, Pose2d poseVelocity) {
        ...
        if (currentTrajectorySequence != null) {
            ...
            if (isNewTransition) {
                ...
                dispatchCallback(Event.NEW_SEGMENT);
            }
        ...
        packet.put("xDesired", targetPose.getX());
        packet.put("yDesired", targetPose.getY());
        packet.put("headingDesired", Math.toDegrees(targetPose.getHeading()));
